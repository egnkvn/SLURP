...
def parse_arguments():
    ...
    parser.add_argument('--golden_text_path', type=str, default='./pipline_method/golden_datamap_labels.json')
...
def main(args):
    ...
    golden_text_json = pd.read_json(args.golden_text_path)
    ...
    err = dict()
    err['pink'] = list(e2e_hard.intersection(pipeline_hard, golden_text_hard))
    err['purple'] = list(purple.difference(err['pink']))
    err['green'] = list(green.difference(err['pink']))
    err['orange'] = list(orange.difference(err['pink']))
    err['red'] = list(e2e_hard.difference(purple, orange))
    err['yellow'] = list(golden_text_hard.difference(orange, green))
    err['blue'] = list(pipeline_hard.difference(green, purple))
