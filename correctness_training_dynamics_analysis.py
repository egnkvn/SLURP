...
def relabel(training_dynamics, file_names, ambig_thres, hard_thres, easy_thres):
    ...
    conf_btm33 = np.percentile(training_dynamics["correct_means"], hard_thres)    # Hard
    conf_top33 = np.percentile(training_dynamics["correct_means"], easy_thres)    # Easy
    ...
    for idx in range(len(file_names)):
        ...
        if training_dynamics["correct_means"][idx] <= conf_btm33:         # Hard
            relabeled_data['hard'].append(file_names[idx])
        if training_dynamics["correct_means"][idx] >= conf_top33:         # Easy
