...
def main():
    ...
    class DatamapTrainer(Trainer):
        ...
        def training_step(
            ...
            for input_id in inputs['input_ids']:
                ...
                fname = ''.join(tokens).replace('#', '')
                match_result = re.search('<s>(.*)<.s>', fname , re.I|re.M)
                fname = match_result.group(1)

                self.fname_inputs.append(fname)
    ...
    if training_args.do_train:
        ...
        with open('./fname_5_2.json', 'w') as writer:
